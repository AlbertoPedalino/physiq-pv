...
Questo file spiega **perché** il modello è fatto così. Per **dove** vive il
codice e chi chiama chi: **[CODE_FLOW_MTGFLOW.md](CODE_FLOW_MTGFLOW.md)**.

Codice: `physiq_pv/anomaly_detection/mtgflow/` (`model.py`, `pipeline.py`,
`config.py`) e `physiq_pv/anomaly_detection/thresholds.py`.
...
## 2. Input: normalizzazione e finestre (Eq. 5)

Serie multivariata con `K` entità — qui 3, fissate in `CLIMATE_FEATURES`
(`pvgis_climate.py:22`): `solar_irradiance_poa`, `temperature_2m`,
`wind_speed_10m` — campionate ogni ora.

Normalizzazione per entità, statistiche calcolate **solo sul training**
(`pipeline.py:113-119`); `std` degenere forzato a 1:

```python
x_k = (x_k - mean(x_k)) / std(x_k)
```

Il target di regressione `pv_power_output` è escluso di proposito: il detector
modella condizioni meteo-solari, non produzione. La guardia è
`detector_features` (`common.py:15`), che solleva eccezione se nel frame
compaiono `pv_power_output`, `label`, `is_anomaly`, `anomaly_label` o `attack`.

Finestra scorrevole `M = 60` ore con stride `S` (`Windows`, `pipeline.py:121`):
...
anomala se contiene almeno un punto anomalo. Lo score viene registrato sul suo
istante finale (`endpoints`, `pipeline.py:139`).
...
le dipendenze tra entità evolvono nel tempo, invece di essere un DAG statico
come in GANF. Proiezioni lineari senza bias (`model.py:27-28`), scala
`sqrt(window_size * input_size)` (`:30`), dropout 0.2 dopo il softmax (`:38`) —
attivo solo in `train()`.

Non c'è softmax sulle colonne né simmetrizzazione: `a_ij` quantifica il flusso
da `j` verso `i` e la riga `i` somma a 1.
...
Nessuna etichetta, nessun early stopping, nessun model selection: epoche fisse
(`checkpoint_selection = "final_fixed_epoch"`, `pipeline.py:205`). Ottimizzare
tutti i moduli insieme evita che grafo e flow convergano a ottimi locali
separati.
...
## 9. Schema del modello

```text
finestre M=60, stride S → (B,K,M,1)
   │
...
## 10. Confronto con l'etichettatore climatologico

Nel progetto convivono due definizioni indipendenti di "anomalia".
...
segnalate da MTGFlow e ignorate dalla climatologia.

Implementazione climatologica: `physiq_pv/data/pvgis_anomaly_scores.py`.
Chi consuma quale output: `CODE_FLOW_MTGFLOW.md` §7.

## 11. Fedeltà e scelte non specificate dal paper
