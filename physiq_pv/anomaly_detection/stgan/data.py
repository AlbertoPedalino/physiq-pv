...
from .graph import GeographicalSubgraphs
...
class STGANWindowDataset(Dataset):
    ...
    def __init__(
        ...
        data: np.ndarray,
        timestamps: pd.DatetimeIndex,
        subgraphs: GeographicalSubgraphs,
        *,
        ...
        if data.shape[1] != subgraphs.n_locations:
            raise ValueError("STGAN graph does not match the data location axis.")
    ...
    def __getitem__(self, item: int):
        ...
        nodes = self.subgraphs.node_indices[location_index]
        recent = self._normalise(
            np.asarray(
                self.data[target_time - self.recent_steps : target_time, nodes, :]
            )
        )
        ...
        observed = self._normalise(np.asarray(self.data[target_time, nodes, :]))
        adjacency = self.subgraphs.normalized_adjacency[location_index]
        return (
            torch.from_numpy(np.ascontiguousarray(recent)),
            torch.from_numpy(np.ascontiguousarray(trend)),
            torch.from_numpy(np.ascontiguousarray(adjacency)),
