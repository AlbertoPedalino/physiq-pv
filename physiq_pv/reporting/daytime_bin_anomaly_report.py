...
def build_frequency_weighted_bin_summary(
    ...
    """Summarise bin reliability weighted by observed category frequency."""
    rows = []
    for category in CATEGORY_ORDER:
...
def render_report(args, col, stats, day, overview, bin_summary,
    ...
    L.append("- Predictive interval: saved `lower_pi`/`upper_pi` from the "
             "selected uncertainty method (MC Dropout or Deep Ensemble).")
    ...
    L.append(
        f"- Run config (reported, not read from the CSV): "
        f"method={args.method_label}, model_type={args.model_type}, "
        f"feature_set={args.feature_set}, epochs={args.epochs}, "
        f"dropout={args.dropout}, mc_samples={args.mc_samples}, "
        f"seed={args.seed}.\n"
    )
    ...
    # 5. Frequency-weighted bin reliability
    L.append("## 5. Frequency-weighted bin reliability\n")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--reference-peak-quantile", "--reference_peak_quantile",
                   ...
                   help="Global daytime y_true quantile used as the fixed "
                        "PVGIS reference peak for production-percentage bins.")
    ...
    p.add_argument("--mc-samples", "--mc_samples", type=int, default=10,
                   help="MC samples or ensemble members (header/provenance only).")
    p.add_argument("--method-label", "--method_label", default="mc_dropout",
                   help="Uncertainty method label shown in the report header.")
    p.add_argument("--model-type", "--model_type", default="stgnn_enhanced_dropout",
                   help="Model type shown in the report header.")
    p.add_argument("--feature-set", "--feature_set", default="full",
                   help="Feature set shown in the report header.")
    p.add_argument("--seed", type=int, default=1,
                   help="Seed shown in the report header.")
    return p.parse_args()
