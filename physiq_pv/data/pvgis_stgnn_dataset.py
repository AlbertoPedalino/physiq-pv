...
def compute_metrics(predictions: pd.DataFrame) -> pd.DataFrame:
    """Global MAE/RMSE over all (location, timestamp) predictions."""
    return pd.DataFrame([_metric_row("all", predictions)])


def build_wandb_metrics(global_df: pd.DataFrame) -> dict:
    """Flatten global metrics into namespaced W&B scalars (mae/global, rmse/global)."""
    g = global_df.iloc[0]
    return {"mae/global": float(g["MAE"]), "rmse/global": float(g["RMSE"])}


def _render_report(global_df: pd.DataFrame, meta: dict) -> str:
    lines: List[str] = []
    lines.append("# PVGIS-only ST-GNN forecasting report\n")
    lines.append(
        "Reuses the existing STGNN architecture on a **PVGIS-only** input. No real "
        "plant production, no ENERGIA, no quality score, no kWp/UPN.\n"
    )
...
def write_outputs(
    ...
    """Write metrics_global.csv + report.md (+ predictions.csv unless `skip_predictions`)."""
    out = Path(out_dir)
    ...
    paths["report"].write_text(_render_report(global_df, meta), encoding="utf-8")
    return paths
